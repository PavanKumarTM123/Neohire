...
export default function JobDetails() {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 sm:p-8 bg-white shadow-lg rounded-lg mt-10 border border-gray-200">
      ...
      <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 mb-2">{job?.jobTitle || "No Title Available"}</h1>
      <p className="text-blue-600 text-lg font-semibold mb-1">{job?.companyName || "No Company Name"}</p>
      <p className="text-gray-500 text-sm mb-4 border-b pb-4">{job?.JobID}</p>
      
      <div className="mt-6 space-y-3 bg-gray-50 p-4 rounded-md">
        <p className="text-gray-700 flex items-center"><span className="w-32 sm:w-40 inline-block font-medium">📍 Location:</span> {job?.location || "Not specified"}</p>
        <p className="text-gray-700 flex items-center"><span className="w-32 sm:w-40 inline-block font-medium">💰 Salary:</span> {job?.ctc ? `${job.ctc} LPA` : "Not specified"}</p>
        <p className="text-gray-700 flex items-center"><span className="w-32 sm:w-40 inline-block font-medium">💰 Job ID:</span> {job?.jobId || "Not specified"}</p>
        <p className="text-gray-700 flex items-center"><span className="w-32 sm:w-40 inline-block font-medium">🔹 Type:</span> {job?.jobType || "Not specified"}</p>
        <p className="text-gray-700 flex items-center"><span className="w-32 sm:w-40 inline-block font-medium">📌 Positions:</span> {job?.vacancy || "Not specified"}</p>
        <p className="text-gray-700 flex items-start"><span className="w-32 sm:w-40 inline-block font-medium">🛠 Skills Required:</span> {job?.requiredSkills || "Not specified"}</p>
      </div>
      
      <div className="mt-8">
        <h2 className="text-xl sm:text-2xl font-semibold mb-3 text-gray-800">Job Description</h2>
        <div className="bg-gray-50 p-5 rounded-md border border-gray-200">
          <p className="text-gray-700 whitespace-pre-line leading-relaxed">{job?.jobDescription || "No description available."}</p>
        </div>
      </div>
